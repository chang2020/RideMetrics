    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2">{t("groups.description")}</p>
        </div>
        <CreateGroupModal>
      ...
      {groups.length === 0 ? (
        <div className="text-center py-12">
          <div className="max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t("groups.noGroups")}</h3>
            <p className="text-gray-600 mb-6">{t("groups.createFirstGroup")}</p>
            <CreateGroupModal>
            ...
            <Card key={group.id} className="hover:shadow-lg transition-shadow cursor-pointer" data-testid={`group-card-${group.id}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        <Badge variant="secondary" data-testid="text-group-visibility">
                          {group.visibility === "public" ? t("groups.public") : group.visibility === "private" ? t("groups.private") : t("groups.inviteOnly")}
                        </Badge>
              ...
              <CardContent>
                <p className="text-gray-600 text-sm mb-4" data-testid="text-group-description">
                  {group.description || t("general.noDescription")}
                </p>
                <div className="flex items-center justify-between text-sm text-gray-500">
                  <div className="flex items-center space-x-1">
                    <Users className="h-4 w-4" />
                    <span data-testid="text-group-member-count">1{t("groups.member")}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <TrendingUp className="h-4 w-4" />
                    <span data-testid="text-group-weekly-distance">{t("groups.thisWeek")} 0km</span>
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="mt-12">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">{t("groups.suggestedGroups")}</h2>
        <div className="text-center py-8 bg-gray-50 rounded-lg">
          <p className="text-gray-500">{t("groups.noSuggestions")}</p>
          <p className="text-sm text-gray-400 mt-2">{t("groups.moreSuggestions")}</p>
        </div>
      </div>
    </div>
  );
